...
def parse_args():
    ...
    parser.add_argument("--model_path", type=str, default="../outputs/pretrain", help="预训练模型路径，可以是本地路径或Hugging Face Hub上的模型ID")
    ...
    parser.add_argument("--output_dir", type=str, default="../outputs/sft", help="模型输出目录")
    parser.add_argument("--data_file", type=str, default="../data/minimind/sft_mini_512.jsonl", help="训练数据文件路径")
    ...
    parser.add_argument("--wandb_project", type=str, default="minimind-sft", help="wandb项目名称")
...
def main():
    ...
    if args.use_wandb and local_rank <= 0:  # 只在主进程上初始化wandb
        wandb.init(project=args.wandb_project, name=f"sft-{os.path.basename(args.output_dir)}")
    ...
    if args.data_limit is not None:
        ...
        dataset["train"] = dataset["train"].select(range(args.data_limit))
    else:
